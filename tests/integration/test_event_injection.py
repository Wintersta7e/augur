...
class TestEventInjection:
    ...
    async def test_inject_creates_baseline(
        ...
        found = await wait_for_redis_pattern(
            redis_client, "augur:profile:inttest:player1"
        )
        assert found, "Baseline key not created within timeout"

        raw = redis_client.get("augur:profile:inttest:player1")
    ...
    async def test_multiple_events_update_baseline(
        ...
        found = await wait_for_redis_pattern(
            redis_client, "augur:profile:inttest:player2"
        )
        assert found, "Baseline key not created within timeout"

        raw = redis_client.get("augur:profile:inttest:player2")
        assert raw is not None
        state = json.loads(raw)
    ...
    async def test_new_domain_creates_baseline(
        ...
        found = await wait_for_redis_pattern(
            redis_client, "augur:profile:newdomain:entity1"
        )
    ...
    async def test_event_persisted_to_history(
        ...
        raw_list = redis_client.lrange("augur:history:inttest", 0, -1)
